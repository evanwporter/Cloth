...
public:
    ...
    Derived operator[](slice<Eigen::Index>& overlay) {
        ...
        return Derived(parent().derived(), combined_mask);
    }
...
public:
    ...
    Series head(Eigen::Index n) const;

    Series tail(Eigen::Index n) const;
...
public:
    ...
    class IlocProxy : public IlocBase<DataFrame> {
    private:
        DataFrame& parent_;
    ...
    Eigen::Map<MatrixXdRowMajor, Eigen::Unaligned, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>> values() {
    ...
    DataFrame head(Eigen::Index n) const;

    DataFrame tail(Eigen::Index n) const;
...
// DataFrame DataFrame::IlocProxy::operator[](const nb::slice& nbSlice) const {
...
//     auto combined_mask = std::make_shared<slice<Eigen::Index>>(combine_slices(*parent.mask_, *overlay));
//     return DataFrame(parent, combined_mask);
// }

// DataFrame DataFrame::IlocProxy::operator[](slice<Eigen::Index>& overlay) const {
//     overlay.normalize(parent.rows());
//     auto combined_mask = std::make_shared<slice<Eigen::Index>>(combine_slices(*parent.mask_, overlay));
//     return DataFrame(parent, combined_mask);
// }

// DataFrame DataFrame::head(Eigen::Index n) const {
//     return iloc()[slice<Eigen::Index>(0, std::min(n, rows()), 1)];
// }

// DataFrame DataFrame::tail(Eigen::Index n) const {
...
NB_MODULE(cloth, m) {
    ...
    nb::class_<Series, Frame<Series>>(m, "Series")
